...
cv::Ptr<cv::aruco::Dictionary> create_dictionary()
{
    ...
    {
        auto code = codes[i];
        cv::Mat mb(6, 6, CV_8UC1, cv::Scalar::all(0));
        for(int x = 0; x < 6; x++)
        {
            for(int y = 0; y < 6; y++)
            {
                mb.at<uchar>(5 - x, 5 - y) = code & 1;
                code >>= 1;
            }
        }
        dictionary->bytesList.push_back(cv::aruco::Dictionary::getByteListFromBits(mb));
    }
    return dictionary;
}
